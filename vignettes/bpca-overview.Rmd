...
retained <- sapply(
  methods,
  function(m) {
    bp <- bpca(gabriel1971, method = m, d = 1:2)
...
```{r}
bp2_q <- bpca(gabriel1971, d = 1:2, var.rb = TRUE)
...
```{r}
bp2_rd <- bpca(gabriel1971, d = 1:2, var.rb = TRUE, var.rd = TRUE, limit = 3)
bp2_rd$var.rd
```
...
```{r}
bp3 <- bpca(gabriel1971, d = 1:3, var.rb = TRUE)
bp3$number
...
# Evaluate one object
plot(bpca(ontario), type = "eo", obj.id = 7)

# Evaluate one variable
plot(bpca(ontario), type = "ev", var.id = "E7")

# Compare two objects
plot(bpca(ontario), type = "co", obj.id = c("g7", "g13"))
```
...
To generate LaTeX output with explicit formatting:

```{r, eval = FALSE}
tbl <- xtable::xtable(bp2, caption = "Biplot summary", label = "tab:bp2")
xtable::print.xtable(tbl)
```
