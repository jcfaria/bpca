           ...
           xlim, ylim, xlab, ylab, ...)
  {
    ...
    draw.circles <-
      ...
      {  
        ...
        r <- max((max(covar[d1,]) -
                 ...
                 (max(covar[d2,]) -
                  min(covar[d2,]))) / 10
        for (i in 1:circles.number)
          symbols(x=0,
                  y=0,
                  circles=r*i*var.factor,
                  add=TRUE,
                  inches=FALSE,
                  fg=circles.color,
                  lwd=circles.lwd, ...)
    ...
    switch(match.arg(type), bp = { # basic biplot 2d
           ...
           if(obj.identify)
             ...
             abline(a=0,
                    b=-(coobj[obj.id[1],d1] - coobj[obj.id[2],d1]) /
                    (coobj[obj.id[1],d2] - coobj[obj.id[2],d2]),
                    col=base.color,
                    lty=base.lty, ...)
               ...
               while (is.na(indice[i + 1]) == FALSE) {
                 ...
                 xint <- ifelse(xint < 0,
                                min(covar[,d1], coobj[,d1]),
                                max(covar[,d1], coobj[,d1]))
               ...
               {
                 x <- solve(matrix(c(-m2, m1, m1, m2),
                                   nrow=2),
                            matrix(c(0, m2 * coobj[i,d2] +
                                     m1*coobj[i,d1]),
                                   ncol=1))
               ...
               for (i in 1:nrow(coobj))
               {
                 x <- solve(matrix(c(-m2, m1, m1, m2),
                                   nrow=2),
                            matrix(c(0, m2*coobj[i,d2] + m1 * coobj[i,d1]),
                                   ncol=1))
                 if (sign(x[1]) == sign(m1))
                   if(abs(x[1]) > abs(cox))
                   {
